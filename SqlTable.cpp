...
bool Table::index(string indexName, string fieldName, bool unique) {
    std::vector<string> fieldNames;
    fieldNames.push_back(fieldName);
    return index(indexName, fieldNames, unique);
}

bool Table::index(string indexName, std::vector<string> fieldNames, bool unique) {
    
    assert(fieldNames.size() > 0);
    std::vector<string>::iterator iter = fieldNames.begin();
    string fields = *iter;
    ++iter;
    ...
    string queryStr;
    if (unique) {
        queryStr = "CREATE UNIQUE INDEX IF NOT EXISTS ";
    } else {
        queryStr = "CREATE INDEX IF NOT EXISTS ";
    }
    queryStr += indexName;
    queryStr += " ON ";
    queryStr += _tableName;
...
bool Table::updateRecord(Record* record, string condition)
{
	if (record)
	{
		const string sql = record->toSqlUpdate(name(), condition);
